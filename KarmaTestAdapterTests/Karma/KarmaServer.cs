...
namespace KarmaTestAdapterTests.Karma.KarmaServerTests
{
    public abstract class KarmaServerFixture : BaseFixture
    {
        ...
        public override void Init()
        {
            ...
            var settings = new KarmaTestAdapter.TestAdapter.KarmaSettings("Test", karmaConfFile, f => File.Exists(f), TestProjectsDir, Logger);
    ...
    public class KarmaServer : KarmaServerFixture
    {
        ...
        {
            var port = await Server.StartServer(60000);
            var eventsCommand = new JsTestAdapter.TestServerClient.EventCommand(port);
        ...
        {
            var port = await Server.StartServer(60000);
            var stopCommand = new JsTestAdapter.TestServerClient.StopCommand(port);
